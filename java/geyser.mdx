...
1. **Floodgate**: If you use the Floodgate plugin, Bedrock players can join directly without logging into a Microsoft account. Just enable the "auth-type: floodgate" option in the GeyserMC **config.yml** file.
   ```yaml
   remote:
   auth-type: floodgate
   ```
